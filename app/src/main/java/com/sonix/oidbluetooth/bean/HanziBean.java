...
public class HanziBean {
    ...
    private void initMedians(List<List<List<Integer>>> medians){
        medianPaths.clear();
        medianPath.reset();
        Log.e("dbj","initMedians--medians="+medians.size());
        for(List<List<Integer>> median : medians){
            Path path = null;
            for(List<Integer> coordinate : median){
                Log.e("dbj","initMedians--x="+getCoordinateX(coordinate.get(0)));
                Log.e("dbj","initMedians--y="+getCoordinateX(coordinate.get(1)));
                if(path == null){
                    path = new Path();
                    path.moveTo(
